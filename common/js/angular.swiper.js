        ...
        return {
            ...
            controller: function($scope, $element, $timeout) {
                ...
                if (!angular.isUndefined($scope.autoplay) && typeof $scope.autoplay === 'number') {
                    //�������1�� �Զ��ֲ�
                    if(!angular.isUndefined($scope.slideNums) && $scope.slideNums > 1){
            ...
            '<div class="parallax-bg" data-swiper-parallax="{{parallaxTransition}}" ng-if="parallax"></div>' +
            '<div class="swiper-wrapper {{wrapperCls}}" ng-transclude></div>' +
            '<div class="swiper-pagination {{paginationCls}}"></div>' +
            '<div class="swiper-button-next" ng-if="showNavButtons"></div>' +
            '<div class="swiper-button-prev" ng-if="showNavButtons"></div>' +
            '<div class="swiper-scrollbar" ng-if="showScrollBar"></div>' +
            '</div>'
        };
    }
